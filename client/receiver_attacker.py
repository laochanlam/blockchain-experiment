...
def main():
    ...
    # POW
    w1 = threading.Thread(target=proof_of_work,args=(public_key, block_chain, tx_pool,s,(network,port),0))
    w2 = threading.Thread(target=proof_of_work,args=(public_key, block_chain, tx_pool,s,(network,port),1))
    w3 = threading.Thread(target=proof_of_work,args=(public_key, block_chain, tx_pool,s,(network,port),2))
    w1.start() 
    w2.start()
    w3.start() 
        ...
        if rs != []:
            ...
            try:
                ...
                with open('info.log', 'a') as f:
                    f.write('%s\t%s\t%s\t%s\n' % (str(os.getpid()) + 'A', 'receive', str(receive['index']), str(receive_time)))
                ...
                if has == receive['pre_hash']:
                    ...
                    # 出现分叉！
                    with open('info.log', 'a') as f:
                        f.write('%s\t%s\t%s\n' % (str(os.getpid()) + 'A', 'fork', str(receive['index'])))
                    print('fork!')
